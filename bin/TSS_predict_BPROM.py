...
def process_bprom_output_file(input_file_name, is_reverse_complement, sequence_length):
    ...
    for line in lines:
        ...
        if (i == 1):
            ...
            new_entry["TSSpos"] = int(split_line[2]) if not is_reverse_complement else sequence_length - int(
                split_line[2]) + 1
            new_entry["score"] = split_line[4]
        elif (i == 2):
            new_entry["box10pos"] = int(split_line[4]) if not is_reverse_complement else sequence_length - int(
                split_line[4]) + 1
            new_entry["box10seq"] = split_line[5]
        elif (i == 3):
            new_entry["box35pos"] = int(split_line[4]) if not is_reverse_complement else sequence_length - int(
                split_line[4]) + 1
...
def TTS_predict_BPROM(input, output):
