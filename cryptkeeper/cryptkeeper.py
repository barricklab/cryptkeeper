...
def make_logger(output=None):
    ...
    if output:
        ...
        file_formatter = logging.Formatter(
            "[%(asctime)s - %(levelname)s] Cryptkeeper: %(message)s"
        )
...
def cryptkeeper(
    ...
    if output:
        ...
        if os.path.isfile(output_folder):
            raise ValueError(
                f"Output folder {output_folder} is a file. Please provide a folder path."
            )
    ...
    if len(sequences) > 1:
        logger.warning(
            "Input file contains multiple sequences. Only the first will be processed."
        )
    ...
    if circular:
        ...
        for start_codons, stop_codons in [
            (fwd_start_codons, fwd_stop_codons),
            (rev_stop_codons, rev_start_codons),
        ]:  # Invert start and stop codons for reverse strand
        ...
        with open(
            output_circular_fasta_file_name, "w", encoding="utf-8"
        ) as output_handle:
            sequences[0].seq = (
                sequences[0].seq[-circular_length:]
                + sequences[0].seq
                + sequences[0].seq[:circular_length]
            )
    ...
    if input_file_type in ["genbank", "gb", "gbk", "gbff"]:
        ...
        delayable_features = delay_iterator(
            features
        )  # Works like appending to a list you're looping through, but lets you know when you're getting delayed objects
        for feature, is_delayed in delayable_features:
            ...
            if feature_type in translation_names:
                for gene in features_list:
                    if (
                        gene.type == "gene"
                        and feature.location.start == gene.start
                        and feature.location.end == gene.end
                    ):
                        break
                else:
            ...
            for i, feature in enumerate(strand):
                ...
                for level, position in end_positions.items():
                    ...
                    if position == "inf":
                        ...
                        break
                if (
                    not strand[i].nest_level and strand[i].nest_level != 0
                ):  # Layer 0 is falcey
    ...
    rhotermpredict_predictions = rho_term_predict(
        inseq=[str(main_seq.seq)]
    )  # Run RhoTermPredict
    # Load RhoTermPredict output into named tuple
    rdtresult = namedtuple(
        "rdtresult",
        """strand,
                                        c_over_g,
                                        start_rut,
    ...
    for orf in orfs:
        ...
        if orf["start"] + offset in expressed_starts.keys():
            ...
            array = (
                max(
                    int(orf["end"]) - int(orf["start"]),
                    int(orf["start"]) - int(orf["end"]),
                )
                + 1
            )
            processed_orf = expressed_orf(
                int(orf_info["start"]) - circular_length,
    ...
    if circular:
        ...
        expressed_orfs = list(
            filter(lambda x: (0 <= int(x.start) < len(single_sequence)), expressed_orfs)
        )

        promoter_calc_predictions = list(
            filter(
                lambda x: 0 < int(x.TSSpos) < len(single_sequence),
                promoter_calc_predictions,
            )
        )

        transterm_predictions = list(
            filter(
                lambda x: 0 < int(x.start) < len(single_sequence), transterm_predictions
            )
        )

        rhotermpredict_results = list(
            filter(
                lambda x: 0 < int(x.start_rut) < len(single_sequence),
                rhotermpredict_results,
            )
        )
